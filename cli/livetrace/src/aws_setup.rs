...
pub async fn setup_aws_resources(args: &CliArgs) -> Result<AwsSetupResult> {
    ...
    if group_count == 0 {
        let error_msg = if args.stack_name.is_some() {
             ...
             format!("Pattern '{}' matched 0 valid log groups (checked Lambda@Edge variants).", args.log_group_pattern.as_deref().unwrap_or("N/A"))
        };
        ...
        let (method, value) = if let Some(stack) = args.stack_name.as_deref() {
            ...
            ("Pattern", args.log_group_pattern.as_deref().unwrap_or("N/A"))
        };
        let error_msg = format!("{} '{}' resulted in {} valid log groups (max 10 allowed for live tail). Found: {:?}",
...
async fn discover_log_group_names(
    ...
    args: &CliArgs,
     ...
     if let Some(stack_name) = args.stack_name.as_deref() {
        ...
        Err(anyhow::anyhow!("Internal error: No log group pattern or stack name provided."))
    }
}

/// Discovers log groups within a CloudFormation stack.
async fn discover_log_groups_from_stack(cfn_client: &CfnClient, stack_name: &str) -> Result<Vec<String>> {
...
async fn discover_log_groups_by_pattern(cwl_client: &CwlClient, pattern: &str) -> Result<Vec<String>> {
